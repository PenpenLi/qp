...
cc.Class({
	...
	onLoad: function() {
		...
		this.tick = GameProto.AUTO_SORTCARD_TM;
	...
	setCardArr: function(cardArr) {
		...
		for(i = 0; i < this.cardArr.length; ++i) {
			Tools.updateSpriteFrame('ThirteenWater/TWCard/'+this.cardArr[i], this.cardNodeArr[i].getComponent(cc.Sprite));
	...
	sendSortCard: function() {
		...
		Global.NetworkManager.notify(GameMessageRouter, GameProto.getGameCardsSortRequestData(cardArr));
	},
	...
	setTypeNameByCards: function(cardArr, sprite) {
		...
		Global.Tools.updateSpriteFrame(url, sprite);
	...
	update: function(dt) {
		var preTick = this.tick;
		this.tick -= dt;
		if(this.tick < 0) { this.tick = 0; }
		if(Math.floor(preTick) !== Math.floor(this.tick) && Math.floor(this.tick) <= 5) {
			Global.AudioManager.playSound('ThirteenWater/TWSound/tw_daojishi1');
		}
		this.clockLabel.string = Math.floor(this.tick);
	}
});
