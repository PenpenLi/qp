...
cc.Class({
	...
	touchEnd: function(event) {
		var curPos = event.touch._point;
		this.tScrollView.node.setPosition(cc.p(0, 0));
		this.tContent.setPosition(cc.p(0, 0));
		this.bScrollView.node.setPosition(cc.p(0, 0));
		this.bContent.setPosition(cc.p(0, 0));
		if(this.scrollDir === 'horizontal') {
			var offx = curPos.x-this.startPos.x;
			if(Math.abs(offx) >= 73) {
				...
				this.cardSprite.node.runAction(cc.moveTo(0.2, cc.p(0, 0)));
	...
	setCard: function(cardId) {
		...
		for(var i = 0; i < nameArr.length; ++i) {
			...
			Global.Tools.updateSpriteFrame(url, node.getComponent(cc.Sprite));
		}
		Global.Tools.updateSpriteFrame(url, this.cardSprite);
		this.tScrollView.node.setPosition(cc.p(0, 0));
		this.tContent.setPosition(cc.p(0, 0));
		this.bScrollView.node.setPosition(cc.p(0, 0));
		this.bContent.setPosition(cc.p(0, 0));
