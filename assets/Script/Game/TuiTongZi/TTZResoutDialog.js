...
cc.Class({
	...
	start: function () {
		...
		var url = 'TuiTongZi/Common/loseTitle';
		if(showWin) { url = 'TuiTongZi/Common/winTitle'; }
		var self = this;
		cc.loader.loadRes(url, cc.SpriteFrame, function(err, res) {
			if(! err) {
				self.titleNode.getComponent(cc.Sprite).spriteFrame = res;
			} else {
	...
	showCard: function (chooseNode, cardArr, isWin) {
		...
		if(cardArr[1]%10 === 0) {
			...
			url2 = 'TuiTongZi/Common/img_mj_0' + (cardArr[0]%10+1);
		}
		...
		var nameArr = ['Card1', 'Card2', 'BaoziSprite', 'NumLabel', 'DotSprite', 'BanSprite'];
		...
		if(cardArr[0]%10 === cardArr[1]%10) {
			...
			if(count%1 > 0) {
				chooseNode.getChildByName('BanSprite').active = false;
			}
		}
