...
cc.Class({
	...
	start () {
		this.audioManager = this.node.parent.parent.getComponent('TTZMainDialog').getAudioManager();
		this.initSortCard();
	...
	reconnect () {			// 断线重连|中途进游戏
		...
		for(var i = 0; i < this.cardNodeArr.length; ++i) {
			...
			if(gameStatus === TTZProto.GAME_STATUS_WAITING || gameStatus === TTZProto.GAME_STATUS_START
				|| gameStatus === TTZProto.GAME_STATUS_POUR) {
	...
	initSortCard () {
		...
		for(i = 0; i < 40; ++i) {
			...
			if(i%2 === 0) {
				this.cardNodePosArr.push(cc.p((19.5-i/2)*10, 0));
			} else {
				this.cardNodePosArr.push(cc.p((19.5-Math.floor(i/2))*10, 4));
			}
	...
	playSortCardAnimal () {
		...
		var callFunc = function() {
			...
			for(var i = 0; i < self.cardNodeArr.length; ++i) {
				pos = cc.p(self.cardNodePosArr[i].x+offArr[Math.floor(Math.random()*offArr.length)], self.cardNodePosArr[i].y+offArr[Math.floor(Math.random()*offArr.length)]);
	...
	playSendCardAnimal () {
		...
		var sendCardFunc = function() {
			...
			node1.runAction(cc.moveTo(0.05, cc.p(pos1.x - sendCardNodePos.x, pos1.y - sendCardNodePos.y)));
			node2.runAction(cc.moveTo(0.05, cc.p(pos2.x - sendCardNodePos.x, pos2.y - sendCardNodePos.y)));
			index += 2;
		};
		...
		var putCardFunc = function() {
			...
			node1.runAction(cc.moveTo(0.1, cc.p(pos1.x-sortCardNodePos.x, pos1.y-sortCardNodePos.y)));
			node2.runAction(cc.moveTo(0.1, cc.p(pos2.x-sortCardNodePos.x, pos2.y-sortCardNodePos.y)));
			index += 2;
		};
		...
		var getCardFunc = function() {
			...
			var node = self.cardNodeArr[curIndex];
			var pos = cc.p(handNodePos.x-sortCardNodePos.x, handNodePos.y-sortCardNodePos.y);
	...
	setCardType (chooseNode, cardArr) {
