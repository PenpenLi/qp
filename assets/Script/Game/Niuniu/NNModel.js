...
model.setEntryRoomData = function(msg) {
	...
	if(! this.showCardArr || this.showCardArr.length === 0) {
		this.showCardArr = [];
		for(var i = 0; i < this.gameRule.memberCount; ++i) {
...
model.messageCallbackHandler = function(router, msg) {
	if(router === 'RoomMessagePush') {
		...
		if(msg.type === NNProto.CAN_POUR_SCORE_PUSH) {
			...
			this.setMyCardArr(msg.data.cardArr);
			...
			console.log('answerGameResoutPush', msg.data.bankIndex, msg.data.cardsArr, msg.data.finalScoreArr);
			...
			this.answerBankChangePush(msg.data.bankChairId, msg.data.gameStatus);
		}
...
model.answerBankChangePush = function(bankChairId, gameStatus){
	this.bankChairId = bankChairId;
	this.answerGameStatusPush(gameStatus);
};

model.answerRobFreeBank = function(chairId, isRob){
	this.robBankArr[this.getChairIdIndex(chairId)] = (isRob)? 1:0;
};
