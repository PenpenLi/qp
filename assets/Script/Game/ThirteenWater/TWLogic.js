...
proto.getResout = function(cardsArr, guaiArr, gameRule) {
	var memberCount = cardsArr.length;
	var i, j, k;
	if(! guaiArr) {		// 默认无怪牌
		guaiArr = []; 
	...
	for(i = 0; i < guaiArr.length; ++i) {	// 怪牌分 
		if(guaiArr[i]) { 
			guaipaiScoreArr[i] = this.hasYitiaolong(cardsArr[i])? 13:3; 
		}
	}
			...
			for(j = 0; j < cardsArr.length; ++j) {
				if(j !== i && !guaiArr[j]) {
					...
					if(this.compareCards(zhongA, zhongB, this.getTouZhongWeiCardType(zhongA), this.getTouZhongWeiCardType(zhongB))) {
						...
						else if(this.hasSitiao(zhongA)) { score = 6; }
						else if(this.hasTonghuashun(zhongA)) { score = 8; }
					} else {
						score = -1;
						if(this.hasHulu(zhongB)) { score = -2; }
						else if(this.hasSitiao(zhongB)) { score = -6; }
						else if(this.hasTonghuashun(zhongB)) { score = -8; }
					}
	...
	for(i = 0; i < scoresArr.length; ++i) { // 打枪倍率
		for(j = 0; j < scoresArr[i].length; ++j) {
			...
			if(scoresArr[i][j][0] < 0 && scoresArr[i][j][1] < 0 && scoresArr[i][j][2] < 0) { daqiangArr[i][j] = -2; }
			for(k = 0; k < scoresArr[i][j].length; ++k) {
				scoresArr[i][j][k] *= gameRule.baseScore;
			}
		}
	}
	return {
...
/* 有三顺 */
proto.hasSanshun = function(arr) {
	if(arr.length !== 13) { return false; }
	var cards = this.removeCardColor(arr);
	cards.sort(function(a, b) { return b-a; });	/* 大到小 */
	for(var i = 0; i <= 11; ++i) {
		if(cards.indexOf(i) >= 0 && cards.indexOf(i+1) >= 0 && cards.indexOf((i+2)%13) >= 0) {
			var array = [];
			...
			array.splice(array.indexOf((i+2)%13), 1);
			if(this.hasShunzi(array)) {
				var sortArray = [12, 11, 10, 9, 8, 4, 3, 2, 1, 0];	//KQJ109, 5432A
				var hasSanShun = true;
				for(var j = 0; j < sortArray.length; ++j) {
					if(array[j] !== sortArray[j]) {
						hasSanShun = false;
						break;
					}
				}
				if(hasSanShun) { return true; }
				var array1 = this.getShunzi(array)[0];
				for(j = 0; j < array1.length; ++j) {
					array.splice(array.indexOf(array1[j]), 1);
				}
				if(this.hasShunzi(array)) {
					return true;
				}
...
proto.changeToServerData = function(mongoData) {
	...
	var memberCount = mongoData.cardsArr.length; 
	var i, j, k;
...
proto.sortSanshunCard = function(arr) {
	var arrClone = [];
		...
		if(cards.indexOf(i) >= 0 && cards.indexOf(i+1) >= 0 && cards.indexOf((i+2)%13) >= 0) {
			...
			if(this.hasShunzi(array)) {
				...
				if(hasSanShun) { 
					var resArr = [];
					for(k = i+2; k >= i; --k) {
						for(j = 0; j < arrClone.length; ++j) {
							if(arrClone[j]%13 === k%13) {
								resArr.push(arrClone[j]);
								arrClone.splice(j, 1);
								break;
							}
						}
					...
					for(k = 0; k < sortArray.length; ++k) {
						for(j = 0; j < arrClone.length; ++j) {
							if(arrClone[j]%13 === sortArray[k]%13) {
								resArr.push(arrClone[j]);
								arrClone.splice(j, 1);
								break;
							}
				...
				var resout = this.getShunzi(array);
				var array1 = resout[0];
				for(j = 0; j < array1.length; ++j) {
					array.splice(array.indexOf(array1[j]), 1);
				}
				if(this.hasShunzi(array)) {
					...
					arrClone = this.sortCardByCountThenColor(arrClone);
					for(j = 0; j < arrClone.length; ++j) {
						resArr.push(arrClone[j]);
					}
					return resArr;
