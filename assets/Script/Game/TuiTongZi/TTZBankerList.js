...
cc.Class({
	...
	pullMenu () {
		if(this.isOut) {
			...
			this.node.runAction(cc.moveTo(0.2, cc.p(this.originPos.x, this.originPos.y)));
		} else {
			this.isOut = true;
			this.node.stopAllActions();
			this.node.runAction(cc.moveTo(0.2, cc.p(this.originPos.x+640, this.originPos.y)));
			this.fillScrollView();
		}
	},
	...
	fillScrollView () {
		...
		if(banker) {
			...
			item.setPosition(cc.p(0, -80*posIndex));
